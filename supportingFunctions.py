...
# -*- coding: utf-8 -*-
"""
Created on Tue Nov 12 17:07:07 2019
...
def getTestingData():
    print('Reading the data. Please wait...')
    path = './test_datasets/'
    filename='test_pat16_V2.hdf5' #set the correct path here
    tic()
    with h5.File(path + filename,'r') as f:
        org=f['tstOrg'][:]

    filename='R100.hdf5' #set the correct path here
    ...
    print('Now doing undersampling....')
    tic()
    atb=generateUndersampled(org,csm,mask,sigma=.00)
    atb=c2r(atb)
    toc()
    print('Successfully undersampled data!')
    return org,atb,csm,mask
